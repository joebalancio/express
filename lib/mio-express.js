...
exports.delete = function (req, res, next) {
   ...
   * @param {Function} next
   */
  Resource.trigger('request', req, function (err) {
    if (err) return next(err);

    Resource.trigger('request:delete', req, function (err) {
      if (err) return next(err);
      ...
      Resource.get(req.query, function(err, resource) {
        ...
        Resource.delete(req.query, function(err) {
          if (err) return next(err);
...
function mergeParams (Resource, target, params) {
  ...
  Object.keys(params).forEach(function (param) {
    ...
    if (param === 'primary') {
			param = Resource.primaryKey
			value = params.primary;
		}
  ...
  Object.keys(target).forEach(function (param) {
    if (typeof target[param] == 'string' && !isNaN(target[param])) {
      if ((Resource.attributes[param] || {}).coerce === 'number') {
        target[param] = Number(target[param]);
      }
    }
  })
