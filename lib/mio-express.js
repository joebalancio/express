 ...
 *
 * Handlers return [`HttpError`](https://github.com/c9/node-http-error)
 * errors. These should be handled by your downstream error handling middleware.
 *
 * @example
 *
 * ```javascript
...
exports.post = function (req, res, next) {
  ...
  if (!req.is('application/json')) {
    return next(new HTTPError.UnsupportedMediaType());
  }
      ...
      if (req.is('application/json-patch+json')) {
        try {
          ...
          return next(new HTTPError.BadRequest(
            "Failed to apply JSON-Patch because it is malformed or invalid."
          ));
        }
      } else {
